...
from dataloader import Addr_Tx_Data
from collections import defaultdict


def trans2dgl(Tx_id2Input_Addr_id_set, Tx_id2Output_Addr_id_set):
    edge_index_0, edge_index_1 = [], []

    for tx_id in Tx_id2Input_Addr_id_set:
        input_addrs = Tx_id2Input_Addr_id_set[tx_id]
        output_addrs = Tx_id2Output_Addr_id_set[tx_id]
        for input_addr, output_addr in itertools.product(input_addrs, output_addrs):
            edge_index_0.append(input_addr)
            edge_index_1.append(output_addr)
    
    g = dgl.graph((edge_index_0, edge_index_1))
...
def gen_dgl_graph(index1, index2, edge_w=None):
...
class NeighborSampler:
    ...
    def __init__(self, data: Addr_Tx_Data, adj_list: list, tx2output_addr_list: list, tx2input_addr_list: list,
                 sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
        """
        Neighbor sampler.
        :param adj_list: list, list of list, where each element is a list of triple tuple (addr_id, tx_id, timestamp)
        ...
        """
        self.data = data
        self.tx2output_addr_list = tx2output_addr_list
        self.tx2input_addr_list = tx2input_addr_list
        self.sample_neighbor_strategy = sample_neighbor_strategy
        self.seed = seed
        self.tx2time = data.tx_time_step
        ...
        self.node2time = [min(self.nodes_neighbor_times[i]) if len(self.nodes_neighbor_times[i]) > 0 else 0 for i in range(len(self.nodes_neighbor_times))]

        if self.seed is not None:
            self.random_state = np.random.RandomState(self.seed)

    def get_dst_ids(self, src_ids: np.ndarray):
        """
        get destination ids of source ids
        :param src_ids: ndarray, shape (batch_size, ), source ids
        :return:
        """
        return np.array([self.data.Tx_id2Output_Addr_ids[tx_id] for tx_id in src_ids])
    
    def get_node_interaction_times(self, node_ids, task_type):
        """
        get interaction times of nodes in node_ids
        :param node_ids: ndarray, shape (batch_size, ), node ids
        :param task_type: str, task type, can be 'addr classification', 'tx classification', or 'link prediction'
        :return:
        """
        if task_type == 'addr classification':
            return np.array([max(self.nodes_neighbor_times[i]) + 1 if len(self.nodes_neighbor_times[i]) > 0 else 0 for i in node_ids])
        elif task_type == 'tx classification':
            return np.array([max(self.nodes_neighbor_times[i]) + 1 if len(self.nodes_neighbor_times[i]) > 0 else 0 for i in node_ids])
        elif task_type == 'link prediction':
            raise NotImplementedError(f'Not implemented error for task type {task_type}!')
        else:
            raise ValueError(f'Not implemented error for task type {task_type}!')
    ...
    def get_historical_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 20):
        ...
        nodes_neighbor_ids = -np.ones((len(node_ids), num_neighbors)).astype(np.longlong)
        ...
        # ndarray, shape (batch_size, num_neighbors)
        nodes_edge_ids = -np.ones((len(node_ids), num_neighbors)).astype(np.longlong)
        ...
        # ndarray, shape (batch_size, num_neighbors)
        nodes_neighbor_times = -np.ones((len(node_ids), num_neighbors)).astype(np.float32)
...
def get_neighbor_sampler(data: Addr_Tx_Data, addr_mask: np.array, tx_mask: np.array, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
    """
    get neighbor sampler
    :param data: Addr_Tx_Data
    :param sample_neighbor_strategy: str, how to sample historical neighbors, 'uniform', 'recent', or 'time_interval_aware''
    :param time_scaling_factor: float, a hyper-parameter that controls the sampling preference with time interval,
    a large time_scaling_factor tends to sample more on recent links, this parameter works when sample_neighbor_strategy == 'time_interval_aware'
    :param seed: int, random seed
    :return:
    """
    addr_num = data.addr_attrs.shape[0]
    tx_num = data.tx_attrs.shape[0]
    addr_set = set(addr_mask) if addr_mask is not None else None
    tx_set = set(tx_mask) if tx_mask is not None else None
    # the adjacency vector stores edges for each node (source or destination), undirected
    # adj_list, list of list, where each element is a list of triple tuple (addr_id, tx_id, timestamp)
    # the list at the first position in adj_list is empty
    # adj_list = [[] for _ in range(addr_num)]
    adj_list = [[] for _ in range(addr_num)]
    tx2output_addr_list, tx2input_addr_list = [[] for _ in range(tx_num)],  [[] for _ in range(tx_num)]

    for tx_id in data.Tx_id2Input_Addr_ids:
        if tx_set is None or tx_id in tx_set:
            input_addrs = data.Tx_id2Input_Addr_ids[tx_id]
            output_addrs = data.Tx_id2Output_Addr_ids[tx_id]
            for input_addr, output_addr in itertools.product(input_addrs, output_addrs):
                if addr_set is None or output_addr in addr_set:
                    adj_list[input_addr].append((output_addr, tx_id, data.tx_time_step[tx_id]))
                if addr_set is None or input_addr in addr_set:
                    adj_list[output_addr].append((input_addr, tx_id, data.tx_time_step[tx_id]))
            
            tx2output_addr_list[tx_id] = [output_addr for output_addr in output_addrs if addr_set is None or output_addr in addr_set]
            tx2input_addr_list[tx_id] = [input_addr for input_addr in input_addrs if addr_set is None or input_addr in addr_set]

    return NeighborSampler(data=data, adj_list=adj_list, tx2output_addr_list=tx2output_addr_list, tx2input_addr_list=tx2input_addr_list,
                           sample_neighbor_strategy=sample_neighbor_strategy, time_scaling_factor=time_scaling_factor, seed=seed)
